"""
Flame: DG2: Clip → Go To → First / Last / In / Out / Custom Frame.

Media Panel; logic in goto_frame_app.
"""
...
def _scope_visible(selection) -> bool:
    ...
    try:
        ...
        logger.warning(
            "DG2: Clip Go To isVisible error pending=%s: %s",
    ...
    logger.debug(
        "DG2: Clip Go To isVisible pending=%s targets=%s visible=%s",
        dgpy_flame_types.summarize(items),
        len(targets),
        visible,
    )
...
def _resolve_execute_selection(selection) -> list:
    ...
    if pending:
        if execute_items and dgpy_flame_types.summarize(
            ...
            logger.debug(
                "DG2: Clip Go To using isVisible context %s "
                "(execute had %s)",
...
def _run_first(selection=None) -> None:
    ...
    app.go_to_first_frame(_resolve_execute_selection(selection))


def _run_last(selection=None) -> None:
    import goto_frame_app as app

    app.go_to_last_frame(_resolve_execute_selection(selection))


def _run_in(selection=None) -> None:
    import goto_frame_app as app

    app.go_to_in_mark(_resolve_execute_selection(selection))


def _run_out(selection=None) -> None:
    import goto_frame_app as app

    app.go_to_out_mark(_resolve_execute_selection(selection))


def _run_custom(selection=None) -> None:
    import goto_frame_app as app

    app.go_to_custom_frame(_resolve_execute_selection(selection))
