"""
Flame: DG: Clip → Resize All Clips.

Media Panel entry; logic in batch_resize_clips_app.
"""
...
def _scope_visible(selection) -> bool:
    ...
    try:
        ...
        logger.warning(
            "DG: Clip Resize All Clips isVisible error pending=%s: %s",
    ...
    visible = bool(clips)
    logger.debug(
        "DG: Clip Resize All Clips isVisible pending=%s clips=%s visible=%s",
        dgpy_flame_types.summarize(items),
        len(clips),
        visible,
    )
...
def _resolve_execute_selection(selection) -> list:
    ...
    if pending:
        if execute_items and dgpy_flame_types.summarize(
            ...
            logger.debug(
                "DG: Clip Resize All Clips using isVisible context %s "
                "(execute had %s)",
...
def _run(selection=None) -> None:
    ...
    app.resize_all_clips_from_selection(_resolve_execute_selection(selection))
...
def get_media_panel_custom_ui_actions():
    ...
    return dgpy_menu_layout.build_media_panel(
        [
            {
                "layout_key": "clip.resize_all",
                "name": "Resize All Clips",
